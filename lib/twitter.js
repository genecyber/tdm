...
module.exports.sendDM = function (destination_username, msg, cb) {
	...
	loadConfigData(function(err, data) {
		var oauth = JSON.parse(data);
		var twitter = new twitter(CONSUMER_KEY, SECRET, oauth.oauth_token, oauth.oauth_token_secret);
		var callQueue = [];
		var errors = [];
		msgParts.forEach(function(msgPart) {
			callQueue.push(getSendMsgFunction(twitter, msgPart, destination_username));
		});

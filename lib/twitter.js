...
var CONSUMER_KEY, SECRET, TWITTER_KEYS, CONFIG_LOCATION
...
module.exports.init = function(keys, config){
	TWITTER_KEYS = require (keys)
	CONSUMER_KEY = 	TWITTER_KEYS.consumer_key;
	SECRET = TWITTER_KEYS.consumer_secret;	
...
module.exports.authAPP = function (url, token, cb) {
	var oauth = {
		consumer_key: CONSUMER_KEY,
		consumer_secret: SECRET,
		callback: "oob"
...
module.exports.readInbox = function(cb) {
	loadConfigData(function(err, data) {
		...
		var _twitter = new twitter(CONSUMER_KEY, SECRET, oauth.oauth_token, oauth.oauth_token_secret);
		_twitter.get('direct_messages', cb);
	});
};
...
module.exports.readMessage = function(id, cb) {
	loadConfigData(function(err, data) {
		var oauth = JSON.parse(data);
		var _twitter = new twitter(CONSUMER_KEY, SECRET, oauth.oauth_token, oauth.oauth_token_secret);
		_twitter.get('direct_messages/show', "?id=" + id, cb);
	});
};

function saveConfig(params) {
	if (CONFIG_LOCATION) {
		fs.writeFileSync(CONFIG_LOCATION, JSON.stringify(params));
	} else {
...
module.exports.sendDM = function (destination_username, msg, cb) {
	...
	var msgParts = msg.match(/.{1,640}/g);
	...
	loadConfigData(function(err, data) {
		...
		msgParts.forEach(function(msgPart) {
			callQueue.push(getSendMsgFunction(_twitter, msgPart, destination_username));
		});
...
function loadConfigData(cb) {
	if (CONFIG_LOCATION) {		
    	fs.readFile(CONFIG_LOCATION, cb);
	} else {
...
function getSendMsgFunction(twitter, msg, to) {
...
function parseErrors(errors, cb) {
	...
	errors.forEach(function(err, index) {
		if(err) {
			if (JSON.parse(err.data).errors) {
				JSON.parse(err.data).errors.forEach(function(msg) {
					errorMsgs.push(msg);
				});
			} else {
				errorMsgs.push(JSON.parse(err.data));
			}
		}
	});
	cb(errorMsgs);
